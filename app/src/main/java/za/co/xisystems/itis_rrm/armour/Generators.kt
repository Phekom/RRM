...
class Generators {
    ...
    fun generateRandomSpecialCharacters(length: Int): String {
    ...
    fun generateRandomCharacters(length: Int): String {
        val pwdGenerator = Builder().withinRange(48, 57)
            .build()
        return pwdGenerator.generate(length)
    }

    fun generateRandomAlphabet(length: Int, lowerCase: Boolean): String {
        val low: Int
        val hi: Int
        if (lowerCase) {
            low = 97
    ...
    fun getRandomAlphabets(count: Int, upperCase: Boolean): Stream<Char?> {
        var characters: IntStream?
        characters = if (upperCase) {
            random.ints(count.toLong(), 65, 90)
        } else {
            random.ints(count.toLong(), 97, 122)
        }
        return characters.mapToObj(IntFunction { data: Int -> data.toChar() })
    }
