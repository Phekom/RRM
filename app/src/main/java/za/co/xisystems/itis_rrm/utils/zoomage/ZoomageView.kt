...
import za.co.xisystems.itis_rrm.R.styleable
import za.co.xisystems.itis_rrm.utils.zoomage.AutoResetMode.Parser
 ...
 * visible window.
 */
class ZoomageView : AppCompatImageView, OnScaleGestureListener {
     ...
     */
    private var isTranslatable = false
    ...
    private val currentDisplayedWidth: Float
        get() = if (drawable != null) drawable.intrinsicWidth * matrixValues[Matrix.MSCALE_X] else 0.0f
    ...
    private val currentDisplayedHeight: Float
        get() = if (drawable != null) drawable.intrinsicHeight * matrixValues[Matrix.MSCALE_Y] else 0.0f

    private val gestureListener: OnGestureListener = object : SimpleOnGestureListener() {
    ...
    fun init(context: Context, attrs: AttributeSet?) {
        ...
        val values = context.obtainStyledAttributes(attrs, styleable.ZoomageView)
        isZoomable = values.getBoolean(styleable.ZoomageView_zoomage_zoomable, true)
        isTranslatable = values.getBoolean(styleable.ZoomageView_zoomage_translatable, true)
        animateOnReset = values.getBoolean(styleable.ZoomageView_zoomage_animateOnReset, true)
        autoCenter = values.getBoolean(styleable.ZoomageView_zoomage_autoCenter, true)
        restrictBounds = values.getBoolean(styleable.ZoomageView_zoomage_restrictBounds, false)
        doubleTapToZoom = values.getBoolean(styleable.ZoomageView_zoomage_doubleTapToZoom, true)
        minScale = values.getFloat(styleable.ZoomageView_zoomage_minScale, MIN_SCALE)
        maxScale = values.getFloat(styleable.ZoomageView_zoomage_maxScale, MAX_SCALE)
        doubleTapToZoomScaleFactor = values.getFloat(styleable.ZoomageView_zoomage_doubleTapToZoomScaleFactor, 3f)
        autoResetMode = Parser.fromInt(values.getInt(styleable.ZoomageView_zoomage_autoResetMode, AutoResetMode.UNDER))
        verifyScaleRange()
        values.recycle()
    }
    ...
    private fun updateBounds(values: FloatArray) {
        if (drawable != null) {
            bounds[values[Matrix.MTRANS_X], values[Matrix.MTRANS_Y],
